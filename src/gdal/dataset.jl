...
### Parameters
* `eRWFlag`     Either `GF_Read` to read a region of data, or GF_Write to write
a region of data.
...
additional arguments to specify resampling and progress callback, or `NULL` for
default behaviour. The `GDAL_RASTERIO_RESAMPLING` configuration option can also
be defined to override the default resampling to one of `BILINEAR`, `CUBIC`,
