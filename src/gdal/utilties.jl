...
Use the CPL StringList functions such as CSLFetchNameValue() to manipulate it.
...
Returns:
NULL on failure to find the key, or a pointer to an internal copy of the value
string on success.
"""
...
"--version":    Returns one line version message suitable for use in response
                to --version requests. ie. "GDAL 1.1.7, released 2002/04/16"
"LICENSE":      Returns the content of the LICENSE.TXT file from the GDAL_DATA
                directory. Before GDAL 1.7.0, the returned string was leaking
                memory but this is now resolved. So the result should not been
                freed by the caller.
"BUILD_INFO":   List of NAME=VALUE pairs separated by newlines with information
